...
from scripts.aufgabe04.navigation.planning.route_smoothing import smooth_plan_route_results
from scripts.aufgabe04.navigation.planning.station_approach import NavigationTarget, navigation_targets_from_visits
...
def build_station_route_dry_run(
    ...
    *,
    ...
    costmaps = build_station_route_costmaps(
        ...
        transit_keepout_radius_m=transit_keepout_radius_m,
        arena_bounds=selected_arena_bounds,
    )
    targets = navigation_targets_from_visits(visits, costmaps.target_costmap)
    current = start if start is not None else targets[0].pose
    results = []
    for target in targets:
        result = plan_route(
            costmaps.planning_costmap,
            current,
            target.pose,
    ...
    smoothed_results = smooth_plan_route_results(
        tuple(results),
        costmap=costmaps.planning_costmap,
    ...
    return StationRouteDryRun(
        grid=grid,
        base_costmap=costmaps.base_costmap,
        planning_costmap=costmaps.planning_costmap,
        station_map=selected_station_map,
        visits=visits,
        targets=targets,
